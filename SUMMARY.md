...
- **Home aligned to the doorway DNA** (2026-06-03) — kept the cinematic arrival hero (video + sparks + IntroDock) but de-chipped the live label and added the hero scrim; body is full-width (`.home-body`) with mono kickers. Communities section reuses `DoorwayFeature` (immersive card, "Enter {name} →"); fundraisers section reuses `FrontRow` rows styled as cards (`.home-rows`) that navigate to `/f/[id]`. The floating "Replay intro" button was removed — clicking the docked emblem already re-expands the intro. Old `cause-card.tsx`, `community-tile.tsx`, and `reveal.tsx` became unused and were deleted.
...
`pnpm typecheck`, `pnpm lint`, `pnpm test` (67 tests, 20 files), and `pnpm build` all pass. Build prerenders `/`, `/communities`, `/communities/wildfire-watch`, `/f/{alerts,shelter,replant}`, `/u/janahan`.
